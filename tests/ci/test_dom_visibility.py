...
from browser_use.dom.frame_assembler import is_cross_origin_iframe_size_eligible
...
class TestCrossOriginIframeSizeEligibility:
	def test_accepts_wide_secure_card_field(self):
		...
		assert is_cross_origin_iframe_size_eligible(width=250, height=40) is True
	...
	def test_accepts_compact_secure_cvv_field(self):
		...
		assert is_cross_origin_iframe_size_eligible(width=90, height=40) is True

	def test_rejects_tiny_tracking_frame(self):
		"""Pixel-sized web beacons and one-pixel strips must stay excluded."""
		assert is_cross_origin_iframe_size_eligible(width=1, height=1) is False
		assert is_cross_origin_iframe_size_eligible(width=1, height=2500) is False

	def test_accepts_exact_minimum_size(self):
		assert is_cross_origin_iframe_size_eligible(width=10, height=10) is True
	...
	def test_accepts_native_checkbox_sized_frame(self):
		...
		assert is_cross_origin_iframe_size_eligible(width=13, height=13) is True

	def test_rejects_frames_below_the_minimum_edge(self):
		assert is_cross_origin_iframe_size_eligible(width=9, height=100) is False
		assert is_cross_origin_iframe_size_eligible(width=100, height=9) is False
