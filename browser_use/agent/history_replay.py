...
class AgentHistoryReplay:
	...
	async def _update_action_indices(
		...
		if highlight_index is None and historical_element.stable_hash is not None:
			...
			self.agent.logger.debug('STABLE hash match skipped (no stable_hash in history)')
				...
				if attr_key in hist_attrs and hist_attrs[attr_key]:
					for idx, elem in selector_items:
						if (
							...
							self.agent.logger.info(f'Element matched via {attr_key} attribute: {hist_attrs[attr_key]}')
							break
