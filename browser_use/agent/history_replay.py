...
from browser_use.agent.history_loader import HistoryLoader
...
class AgentHistoryReplay:
	"""Owns history replay behavior for an Agent."""

	def __init__(self, agent: Agent) -> None:
		self.agent = agent
		self.loader = HistoryLoader(agent, self)
		self.matcher = ReplayElementMatcher(agent)
		self.retry_policy = ReplayRetryPolicy(agent)
	...
	async def _generate_rerun_summary(
		...
		from browser_use.agent.results import RerunSummaryAction
		...
		try:
			...
			self.agent.logger.warning(f'Failed to capture screenshot for rerun summary: {e}')
		...
		try:
			...
			if summary_llm is None:
				...
				self.agent.logger.debug('Using agent LLM for rerun summary')
			else:
				self.agent.logger.debug(f'Using provided LLM for rerun summary: {summary_llm.model}')
			...
			self.agent.logger.debug(f'Calling LLM for rerun summary with {len(messages)} message(s)')
			try:
				...
				self.agent.logger.debug(f'LLM text response: {response_text}')
			...
			self.agent.logger.info(f'📊 Rerun Summary: {summary.summary}')
			self.agent.logger.info(f'📊 Status: {summary.completion_status} (success={summary.success})')
			...
			self.agent.logger.warning(f'Failed to generate AI summary: {e.__class__.__name__}: {e}')
			self.agent.logger.debug('Full error traceback:', exc_info=True)
	...
	async def _execute_ai_step(
		...
		self.agent.logger.debug(f'Using LLM for AI step: {llm.model}')
		...
		if include_screenshot:
			try:
				...
				self.agent.logger.warning(f'Failed to capture screenshot for ai_step: {e}')
		...
		try:
			...
			self.agent.logger.info(f'🤖 AI Step: {memory}')
			return ActionResult(
			...
			self.agent.logger.warning(f'Failed to execute AI step: {e.__class__.__name__}: {e}')
			self.agent.logger.debug('Full error traceback:', exc_info=True)
	...
	async def rerun_history(
		...
		try:
			for i, history_item in enumerate(history.history):
				...
				self.agent.logger.info(f'Replaying {step_name} ({i + 1}/{len(history.history)}) [{delay_source}]: {goal}')
				...
				if (
					...
					self.agent.logger.warning(f'{step_name}: No action to replay, skipping')
				...
				if self.retry_policy.is_redundant_retry_step(history_item, previous_item, previous_step_succeeded):
				...
				while retry_count < max_retries:
					try:
						...
						if (
							...
							and self.retry_policy.is_menu_opener_step(previous_item)
						):
							...
							if self.retry_policy.is_menu_item_element(curr_elem):
								self.agent.logger.info(
								...
								if reopened:
									...
									self.agent.logger.info('🔄 Dropdown re-opened, retrying element match...')
									continue
			...
			self.agent.logger.info('🤖 Generating AI summary of rerun completion...')
	...
	async def _wait_for_minimum_elements(
		...
		self.agent.logger.warning(f'⚠️ Timeout waiting for {min_elements} elements, proceeding with {last_count} elements')
		return await self.agent.browser_session.get_browser_state_summary(include_screenshot=False)
	...
	async def _execute_history_step(
		...
		for i, action in enumerate(history_item.model_output.action):
			...
			if action_name == 'extract':
				...
				if pending_actions:
					batch_results = await self.agent._execution.multi_act(pending_actions)
				...
				self.agent.logger.info(f'🤖 Using AI step for extract action: {query[:50]}...')
				...
				updated_action = await self.matcher.update_action_indices(
					historical_elem,
					action,
					state,
				)
				if updated_action is None:
					...
					elem_info = self.matcher.format_element_for_error(historical_elem)
	...
	async def _reexecute_menu_opener(
		...
		"""
		try:
			self.agent.logger.info('🔄 Re-opening dropdown/menu by re-executing previous step...')
