"""Page-level LLM querying and extraction."""
...
class PageExtraction:
	"""Find elements and extract structured page content with an LLM."""
	...
	async def get_element_by_prompt(self, prompt: str, llm: BaseChatModel | None = None) -> Element | None:
		...
		enhanced_dom_tree, _ = await dom_service.get_dom_tree(target_id=self.page._target_id, all_frames=None)

		session_id = self.page._browser_session.id
...
Before you return the element index, reason about the state and elements for a sentence or two."""
		...
		element_session = await self.page._browser_session.session_manager.frames.cdp_client_for_node(element)
		return Element_(self.page._browser_session, element.backend_node_id, element_session.session_id)
...
""".strip()
	...
	async def _extract_clean_markdown(self, extract_links: bool = False) -> tuple[str, dict]:
		...
		dom_service = self.dom_service
		return await extract_clean_markdown(dom_service=dom_service, target_id=self.page._target_id, extract_links=extract_links)
