...
class DemoMode:
	...
	def _load_script(self) -> str:
		if self._script_source is None:
			self._script_source = files('browser_use.browser').joinpath('demo_mode.js').read_text(encoding='utf-8')
	...
	async def ensure_ready(self) -> None:
		...
		async with self._lock:
			...
			if self._script_identifier is None:
				self._script_identifier = await self.session.cdp.add_init_script(script)
